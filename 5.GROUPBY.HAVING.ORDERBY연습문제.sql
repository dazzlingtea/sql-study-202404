...
SELECT 
    NVL(department_id, 0) "부서 번호"
    , TO_CHAR(ROUND(AVG(salary), 2), '999,999,999') "평균 급여"
FROM employees
GROUP BY department_id
--HAVING NVL(department_id, 0)
ORDER BY 1
;
...
--    단, 부서번호가 null이면 0으로 변경하여 조회세요.
SELECT NVL(department_id, 0) 부서번호, COUNT(*) "부서별 총 사원 수"
FROM employees
GROUP BY department_id
ORDER BY 1
;
...
SELECT 
    department_id "부서번호"
    , TO_CHAR(ROUND(AVG(salary), 2), '999,999,999') "급여 평균"
FROM employees
GROUP BY department_id
...
SELECT 
    job_id "직무 이름"
    , TO_CHAR(ROUND(AVG(salary), 2), '999,999,999') "직무별 급여 평균"
FROM employees
