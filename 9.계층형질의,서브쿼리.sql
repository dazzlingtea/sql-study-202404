...
-- 그 달 전체 평균 급여보다 높은 사원들의 정보(사번, 이름, 급여지급일, 받은급여액수) 조회
...
SELECT
    COUNT(DISTINCT emp_nm) -- 45건, 이관심 한번만 카운트
FROM tb_emp
;
SELECT
    DISTINCT emp_no || emp_nm -- 46건
FROM tb_emp
;
...
SELECT
    ...
    E.birth_de,
    E.sex_cd
FROM tb_emp E, tb_dept D -- 조인까지 해야돼?
