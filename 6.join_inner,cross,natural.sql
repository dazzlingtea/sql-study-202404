...
SELECT
    ...
    tb_dept.dept_cd,
    tb_dept.dept_nm
FROM tb_emp, tb_dept -- WHERE절 없으면 574건 41 * 14
WHERE tb_emp.dept_cd = tb_dept.dept_cd
;
SELECT
    *
FROM tb_emp, tb_dept -- 컬럼 선택 안하면 모든 컬럼을 가로로 병합(tb_emp 우선)
WHERE tb_emp.dept_cd = tb_dept.dept_cd
;
...
;
-- 사원의 사원번호, 사원명과 취득한 자격증명, 부서명을 조회
SELECT 
    E.emp_no 사원번호
    ,E.emp_nm 사원명
    ,D.dept_nm 부서명
...
--    조인이 일어나지만 USING을 사용하면 원하는 컬럼에 대해서만 선택적 조인조건을 
--    부여할 수 있습니다.
