...
INSERT INTO char_compare VALUES ('104', 'SQLD', '  SQL D');
...
SELECT 
    ...
    REPLACE('Java Programmer Java', 'Java', 'BigData') AS "REPLACE" -- 문자를 변경, Java를 BigData로 변경
    , REPLACE('Java Programmer', 'Java') AS "REPLACE" -- 문자를 제거, Java 찾아서 공백으로 바꾸므로 제거 효과
FROM dual;
...
SELECT 
    ...
    CEIL(38.678) AS ceil, -- 올림값 반환 38.68
    FLOOR(38.678) AS floor, -- 내림값 반환 38.67
...
SELECT 
    SYSDATE AS "현재 시간",
    SYSDATE - 1 AS "SYSDATE - 1",   -- 어제
    (SYSDATE + 10) - SYSDATE AS "날짜 - 날짜", -- 10
    SYSDATE - (1/24) AS "1시간 차감",
    SYSDATE - (1/24/60) * 100 AS "100분 차감", -- (1/24/60) 1분 
...
SELECT
    ...
    TO_CHAR(1350000, 'L999,999,999') AS "원화"  -- L 운영체제에 맞는 화폐기호
FROM dual;
